...
class ChatMessagingConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data = None, bytes_data = None):
        ...
        map_event_type_to_handlers = {
            ...
            "message.acknowleged": self.receive_message_acknowledged_event,
        ...
        try:
            await map_event_type_to_handlers[event_type](data)
        except KeyError:
            error_msg = {"error": "invalid event type"}
            await self.send(text_data=json.dumps(error_msg))

    @event_handler
    async def dispatch_chat_notification(self):
    ...
    async def receive_message_send_event(self, data):
        ...
        self.send_group(group_string, "message.dispatch", message)
    ...
    async def receive_message_read_event(self, data):
        ...
        if isinstance(room_id, set):
            for id in room_id:
                group = GROUP_STRING.format(id)
                await self.send_group(group, "readreceipt.dispatch", message[room_id])
        else:
            group = GROUP_STRING.format(room_id)
    ...
    async def receive_room_create_event(self, data):
        ...
        if "participants" in room:
            ...
            members = subscribers
        group = GROUP_STRING.format(room["id"])
        if members:
    ...
    async def channel_setup(self, user_id: str):
        groups = await fetch_user_groups(user_id)
        if not (groups):
            # Means there's no group at all, create a group for the user using their id
            groups = [USER_OWN_GROUP.format(user_id=user_id)]
